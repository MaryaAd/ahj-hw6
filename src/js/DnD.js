export default class DnD {
  ...
  onMouseUp(evt) {
    ...
    if (
      this.clone &&
      (elemBelow.closest(".col__content") ||
        elemBelow.closest(".col__footer") ||
        elemBelow.closest(".col__header"))
    ) {
  ...
  addGhostEl(evt) {
    ...
    if (elemBelow.closest(".col__content")) {
      if (!elemBelow.closest(".col__content").hasChildNodes()) {
        ...
        if (
          ...
          target.previousElementSibling.dataset.id === this.id
        ) {
          elemBelow
            .closest(".col__content")
            .insertBefore(
              this.clone,
              elemBelow.closest(".col__card").nextElementSibling,
            );
        }
        if (
          target.nextElementSibling &&
          target.nextElementSibling.dataset.id === this.id
        ) {
          elemBelow
            .closest(".col__content")
            .insertBefore(this.clone, elemBelow.closest(".col__card"));
        }
        if (
          evt.pageY >
          window.scrollY +
            top +
            elemBelow.closest(".col__card").offsetHeight / 2
        ) {
          elemBelow
            .closest(".col__content")
            .insertBefore(
    ...
    if (elemBelow.closest(".col__footer")) {
      elemBelow
        .closest(".trello__col")
        .querySelector(".col__content").scrollTop = elemBelow
        .closest(".trello__col")
        .querySelector(".col__content").scrollHeight;
      elemBelow
        .closest(".trello__col")
        .querySelector(".col__content")
        .appendChild(this.clone);
    }
  ...
  onMouseDown(evt) {
    ...
    const cardElement = evt.target.closest(".col__card");

    // Проверяем, что кликнули на карточку, и не на кнопки удаления или подписки
    if (
      !cardElement ||
      classList.contains("card__delete") ||
      classList.contains("card__subBtn")
    ) {
      return;
    }
    this.draggedEl = cardElement; // Инициализируем draggedEl
    this.id = this.draggedEl.dataset.id; // Получаем id карточки
    ...
    // Убираем место откуда забираем карточку
    this.draggedEl.style.margin = "0"; // Убираем отступы, если есть
    this.draggedEl.style.transition = "none"; // Отключаем анимацию для мгновенного эффекта
    this.draggedEl.style.position = "absolute"; // Делаем карточку абсолютной, чтобы она не влияла на поток
    ...
    // Устанавливаем стили для призрачного элемента
